...
> **更新（自動生成パック導入）**
> Bedrock 側でのカスタムアイテム表示は、Paper プラグインが自動生成する最小パック (`packs/geyserextra_auto.zip`) で成立します。各マッピングの `item_texture.json` エントリはベースアイテムのバニラテクスチャを指すため、運営側が BE リソースパックを用意しなくても**ベース見た目で BE プレイヤーから新規アイテムとして識別**でき、クラフトリザルトのプレビューやレシピブックも動作します。PDC 識別子は引き続き「マッピング名の安定化」に推奨ですが、**未設定でも自動生成名で登録される**ため必須ではありません。専用 BE テクスチャを表示したい場合のみ、別途リソースパックを配置してエントリを上書きしてください。
